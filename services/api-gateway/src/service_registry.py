...
class ServiceRegistry:
    ...
    def _load_from_env(self):
        ...
        # Check all environment variables ending with _SERVICE_URL
        for env_var, url in os.environ.items():
            if env_var.endswith('_SERVICE_URL') and url:
                # Convert env var to service name
                # e.g., USERS_SERVICE_URL -> users-service
    ...
    async def _discover_from_cloud_run(self):
        ...
        try:
            ...
            for service in self.client.list_services(parent=parent):
                ...
                # Add service URL (only if not already set from environment)
                if service.uri and service_name not in self.services:
